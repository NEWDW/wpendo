...
//        //这里需要从用户获得，此处只是例子
//        for (i in 0 until 5) {
//            val note =  Note("2017/7/${i + 5}", NoteKind.item_todo, "初始化", i.toString(), null)
//
//            notes?.add(note)
...
//        }
    ...
    override var notes: MutableList<Note>? = super.notes?.filter { it.getKind()==NoteKind.item_todo }?.toMutableList()
    override var titles: MutableList<String>? = super.titles
    override fun registerForNewNote() {
        LiveDataBus.get()
            .with(NoteKind.item_todo.toString(), Note::class.java)
